...
const registerScoreServiceRoutes = (Fastify: FastifyInstance, SCORE_SERVICE: string) => {
	...
	Fastify.get<{ Params: SCORE_GetUserScoreRequestParams, Reply: { scores: SCORE_ScoreDTO[] } | SCORE_ServerErrorReply }>('/score/tournament/user/:user_id', async (request, reply) => {
		const token = request.headers.authorization || '';
